...
// AlgoFin v2 — Phase H: Multi-Exchange support & Compact Single-Page Interface
...
export function BinanceLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
    <div className={`${className} rounded-lg bg-[#0b0e11] border border-[#F0B90B]/30 flex items-center justify-center shrink-0 p-1.5 shadow-md`}>
...
export function BybitLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
    <div className={`${className} rounded-lg bg-[#0d0e12] border border-amber-500/20 flex items-center justify-center shrink-0 p-1 shadow-md select-none`}>
      <span className="font-extrabold text-[10px] tracking-tight text-white font-sans flex items-center">
        BY<span className="inline-block w-[3px] h-3 bg-[#F7A600] rounded-[1px] mx-[1px]" />T
      </span>
    </div>
  );
}

export function CoinbaseLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
    <div className={`${className} rounded-lg bg-[#0052FF] border border-[#0052FF]/50 flex items-center justify-center shrink-0 p-1.5 shadow-md`}>
      <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none">
...
export function DeltaLogo({ className = "w-8 h-8" }: { className?: string }) {
  return (
    <div className={`${className} rounded-lg bg-[#0c0d12] border border-white/10 flex items-center justify-center shrink-0 p-1 shadow-md`}>
      <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none">
...
function ExchangeLogo({ id, name }: { id?: string; name?: string }) {
  ...
  if (key.includes("binance") || key === "b") return <BinanceLogo className="w-8 h-8" />;
  if (key.includes("bybit") || key === "y") return <BybitLogo className="w-8 h-8" />;
  if (key.includes("coinbase") || key === "c") return <CoinbaseLogo className="w-8 h-8" />;
  if (key.includes("delta") || key === "d") return <DeltaLogo className="w-8 h-8" />;

  return (
    <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-xs font-bold text-foreground shrink-0">
...
export default function ExchangesPage() {
  ...
  const defaultExchanges: ExchangeDef[] = useMemo(() => [
    {
      ...
      logo_letter: "B",
      description: "Connect your Binance USDT-M Futures account with read-only API keys for balance, positions and trade sync.",
      api_docs_url: ""
    },
    {
      id: "bybit_linear",
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto space-y-2.5 text-foreground font-sans pb-0">
      {/* ── HEADER ──────────────────────────────────────────────────────── */}
      <div>
      ...
      {/* ── AES-256 SECURITY ALERT BANNER (COMPACT) ──────────────────────── */}
      <div className="bg-[#081b22]/90 border border-cyan-500/25 rounded-xl p-2.5 px-3 flex items-center gap-2.5 shadow-sm">
        ...
        <div className="text-[11px] leading-tight flex flex-wrap items-center gap-x-2">
      ...
      {/* ── TOP STAT CARDS (4-GRID COMPACT) ─────────────────────────────── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
        {/* Card 1: Connected Exchanges */}
        <div className="bg-[#0b101b]/80 border border-white/8 rounded-xl p-2.5 px-3 flex items-center gap-2.5">
        ...
        <div className="bg-[#0b101b]/80 border border-white/8 rounded-xl p-2.5 px-3 flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-1 mt-0.5">
              ...
              <span className="text-[9px] text-muted-foreground/80">Across all</span>
            </div>
          </div>
        </div>
      ...
      {/* ── SEARCH & FILTER CONTROLS BAR (COMPACT) ─────────────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-[#0a0f19]/60 border border-white/6 p-1.5 px-2 rounded-xl">
        {/* Search input */}
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1 rounded-lg text-xs bg-[#090d16] border border-white/8 text-foreground placeholder:text-muted-foreground/50 outline-none focus:border-cyan-500/50 transition-all"
          />
        </div>

        {/* Filter pills */}
        <div className="flex items-center gap-1 bg-[#070a12] p-0.5 rounded-lg border border-white/6">
          {[
            { id: "all", label: "All" },
      ...
      {/* ── MAIN CONTENT GRID (COMPACT SINGLE PAGE) ────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-start">
        {/* ── LEFT COLUMN: 2x2 EXCHANGE CARDS (8 COLS) ────────────────── */}
        <div className="lg:col-span-8 space-y-2.5">
          {/* Skeletons */}
          {loading && showSkeleton && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="bg-[#0b101b] border border-white/8 rounded-xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {filteredExchanges.map(ex => {
                ...
                if (isConnected) {
                  ...
                  return (
                    <div key={ex.id} className="relative bg-[#071918]/80 border-2 border-emerald-500/40 rounded-xl p-3 space-y-2 shadow-md">
                      {/* Header */}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="flex items-center gap-1.5">
                              <h3 className="font-bold text-foreground text-xs tracking-wide">{ex.name}</h3>
                              <span className="text-[9px] font-semibold px-1.5 py-0.2 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">
                                LIVE
                              </span>
                            </div>
                            <p className="text-[10px] text-muted-foreground">{ex.display_name.replace(ex.name, "").trim() || ex.markets.join(" · ")}</p>
                          </div>
                        </div>
                      ...
                      {/* Stats Grid */}
                      <div className="grid grid-cols-4 gap-1 py-1 border-y border-emerald-500/15 text-center">
                        <div>
                          <p className="text-[9px] text-muted-foreground">Balance</p>
                          <p className="text-[11px] font-bold text-foreground">$4,520.21</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-bold text-foreground">2</p>
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-1.5 pt-0.5">
                        <button
                          ...
                          className="flex-1 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/25 text-[11px] font-semibold transition-all flex items-center justify-center gap-1"
                        >
                          ⚙ Manage Account
                        </button>
                        <div className="relative">
                          <button
                            ...
                            className="w-7 h-7 rounded-lg border border-white/10 flex items-center justify-center text-muted-foreground hover:text-foreground text-xs font-bold"
                          >
                            ⋮
                          </button>
                          {activeMenuId === account.id && (
                            <div className="absolute right-0 bottom-full mb-1 w-32 bg-[#0c121e] border border-white/12 rounded-lg p-1 shadow-2xl z-30 text-[10px]">
                              <button onClick={() => handleSync(account.id)} className="w-full text-left px-2 py-1 rounded text-foreground hover:bg-white/5">
                                🔄 Force sync
                              </button>
                              <button onClick={() => handleRevoke(account.id)} className="w-full text-left px-2 py-1 rounded text-rose-400 hover:bg-rose-500/10">
                                🗑 Revoke
                              </button>
                            </div>
                          )}
                ...
                // ── NOT CONNECTED CARD (COMPACT) ─────────────────────
                const isLive = ex.status === "live";

                return (
                  <div key={ex.id} className={`bg-[#0b101b]/80 border rounded-xl p-3 space-y-2 flex flex-col justify-between transition-all ${
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between gap-1.5">
                        ...
                        <span className="text-[9px] font-medium px-2 py-0.5 rounded-full bg-white/5 text-muted-foreground border border-white/8">
                          Not Connected
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-1 text-[9px] text-muted-foreground">
                        <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/6">🔒 Read-only API</span>
                        <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/6">📈 Portfolio Sync</span>
                        <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/6">⏱ Trade History</span>
                      </div>
                    ...
                    <div className="pt-0.5">
                      {isLive ? (
                        <button
                          ...
                          className="w-full py-1.5 rounded-lg bg-cyan-950/40 border border-cyan-500/30 text-cyan-400 text-[11px] font-semibold hover:bg-cyan-500/20 transition-all flex items-center justify-center gap-1"
                        >
                          Connect Account →
                        </button>
                      ) : (
                        <button disabled className="w-full py-1.5 rounded-lg bg-white/3 border border-white/6 text-[10px] text-muted-foreground/60 text-center cursor-not-allowed">
          ...
          {/* Bottom Info Strip */}
          <div className="bg-[#0a0f19]/60 border border-white/6 rounded-xl px-3 py-1.5 flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
            <div className="flex items-center gap-1.5">
        ...
        {/* ── RIGHT COLUMN: SIDEBAR INFO PANEL (COMPACT 4 COLS) ───────── */}
        <div className="lg:col-span-4 space-y-2.5">
          {/* Card 1: How it works (Compact Timeline) */}
          <div className="bg-[#0b101b]/90 border border-white/8 rounded-xl p-3 space-y-2.5">
            <h2 className="text-xs font-bold text-foreground tracking-wide">How it works</h2>

            <div className="relative pl-5 space-y-2.5 before:absolute before:left-[9px] before:top-2 before:bottom-2 before:w-[2px] before:bg-white/10">
              {/* Step 1 */}
              <div className="relative">
                <div className="absolute -left-5 top-0 w-5 h-5 rounded-full bg-purple-500/20 border border-purple-500/40 text-purple-400 text-[10px] font-bold flex items-center justify-center shrink-0">
                  1
                </div>
                <div className="pl-1.5 space-y-0">
                  <h3 className="text-[11px] font-semibold text-foreground">1 Connect Exchange</h3>
              ...
              <div className="relative">
                ...
                <div className="pl-1.5 space-y-0">
                  <h3 className="text-[11px] font-semibold text-foreground">2 Sync Portfolio</h3>
                  <p className="text-[10px] text-muted-foreground leading-tight">
              ...
              {/* Step 3 */}
              <div className="relative">
                <div className="absolute -left-5 top-0 w-5 h-5 rounded-full bg-cyan-500/20 border border-cyan-500/40 text-cyan-400 text-[10px] font-bold flex items-center justify-center shrink-0">
                  3
                </div>
                <div className="pl-1.5 space-y-0">
                  <h3 className="text-[11px] font-semibold text-foreground">3 View Analytics</h3>
                  <p className="text-[10px] text-muted-foreground leading-tight">
              ...
              {/* Step 4 */}
              <div className="relative">
                <div className="absolute -left-5 top-0 w-5 h-5 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-400 text-[10px] font-bold flex items-center justify-center shrink-0">
                  4
                </div>
                <div className="pl-1.5 space-y-0">
                  <h3 className="text-[11px] font-semibold text-foreground">4 Stay Updated</h3>
                  <p className="text-[10px] text-muted-foreground leading-tight">
          ...
          <div className="bg-[#0b101b]/90 border border-white/8 rounded-xl p-3 space-y-2">
            <h2 className="text-xs font-bold text-foreground tracking-wide">Why Connect Your Exchange?</h2>
            ...
            <ul className="space-y-1 text-[10px]">
              {[
                ...
                <li key={benefit} className="flex items-center gap-2 text-muted-foreground">
                  <span className="w-3.5 h-3.5 rounded-full bg-emerald-500/15 text-emerald-400 flex items-center justify-center text-[9px] font-bold shrink-0">
                    ✓
                  </span>
                  <span>{benefit}</span>
                </li>
          ...
          {/* Card 3: Need Help? */}
          <div className="bg-[#0b101b]/90 border border-white/8 rounded-xl p-3 space-y-2">
            <h2 className="text-xs font-bold text-foreground tracking-wide">Need Help?</h2>
            <p className="text-[10px] text-muted-foreground leading-tight">
              Learn how to generate API keys securely.
            </p>
            <button
              onClick={() => setShowDocModal(true)}
              className="w-full py-1.5 rounded-lg bg-[#091a24] border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/10 text-[11px] font-semibold transition-all flex items-center justify-center gap-1"
            >
              View Documentation ↗
            </button>
          </div>

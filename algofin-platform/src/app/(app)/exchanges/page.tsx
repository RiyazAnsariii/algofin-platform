...
export default function ExchangesPage() {
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto flex flex-col text-foreground font-sans gap-4 pb-8">
      {/* ── TOP AREA ────────────────────────────────────────────────────── */}
      <div className="space-y-3.5 shrink-0">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl border border-white/10 text-muted-foreground hover:text-foreground text-xs font-semibold bg-white/5 hover:bg-white/10 transition-all flex items-center gap-1.5 shrink-0 shadow-sm"
          >
            View Documentation ↗
          </button>
        </div>
        ...
        <div className="bg-[#081b22]/90 border border-cyan-500/30 rounded-2xl p-3.5 px-4 flex items-center gap-3.5 shadow-sm">
        ...
        {error && (
          <div className="px-4 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs text-rose-400 font-medium">
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          ...
          <div className="bg-[#0b101b]/90 border border-white/8 rounded-2xl p-3.5 px-4 flex items-center gap-3.5 shadow-md">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-1.5 mt-1">
                <span className="text-lg font-extrabold text-foreground">{connectedCount}</span>
                <span className="text-xs text-muted-foreground">of {supportedCount} supported</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#0b101b]/90 border border-white/8 rounded-2xl p-3.5 px-4 flex items-center gap-3.5 shadow-md">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-1.5 mt-1">
                <span className="text-lg font-extrabold text-foreground">${totalPortfolioValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                <span className="text-[11px] text-muted-foreground/80">Across all exchanges</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#0b101b]/90 border border-white/8 rounded-2xl p-3.5 px-4 flex items-center gap-3.5 shadow-md">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-1.5 mt-1">
                <span className="text-lg font-extrabold text-foreground">{totalPositions}</span>
                <span className="text-[11px] text-muted-foreground/80">Across all exchanges</span>
              </div>
            </div>
          </div>
        ...
        {/* Search & Filter Bar */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#0a0f19]/80 border border-white/8 p-1.5 px-3.5 rounded-2xl shadow-sm">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-1.5 rounded-xl text-xs bg-[#090d16] border border-white/8 text-foreground placeholder:text-muted-foreground/50 outline-none focus:border-cyan-500/50 transition-all"
            />
          </div>
        </div>
      </div>

      {/* ── MAIN CONTENT GRID (Natural Card Height — Zero Empty Black Space!) ─────────── */}
      <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredExchanges.map(ex => {
              ...
              if (isConnected) {
                ...
                return (
                  <div key={ex.id} className="relative bg-[#070c16]/95 border border-[#0d2238] rounded-2xl p-4 sm:p-5 flex flex-col gap-4 shadow-xl transition-all hover:border-[#133252]">
                    {/* Top Header Row */}
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                      <span className="px-3 py-1 rounded-xl bg-[#071828] border border-cyan-500/20 text-cyan-400 font-medium flex items-center gap-1.5">
                        🔒 Read-only API
                      </span>
                      <span className="px-3 py-1 rounded-xl bg-[#071828] border border-cyan-500/20 text-cyan-400 font-medium flex items-center gap-1.5">
                        🔄 Portfolio Sync
                    ...
                    <div className="flex items-end justify-between pt-3 border-t border-white/6 gap-3">
                      <div className="flex items-baseline gap-5 sm:gap-7">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground/80 mt-0.5">Balance</p>
                        </div>
                        <div>
                          <p className="text-base sm:text-lg font-extrabold text-foreground tracking-tight">5</p>
                          <p className="text-xs text-muted-foreground/80 mt-0.5">Positions</p>
                        </div>
                        <div>
                          <p className="text-base sm:text-lg font-extrabold text-foreground tracking-tight">3</p>
                          <p className="text-xs text-muted-foreground/80 mt-0.5">Open Orders</p>
                        </div>
                      </div>
              ...
              // ── NOT CONNECTED CARD (Clean Natural Content Height — Zero Void Space) ───────────
              return (
                <div key={ex.id} className="bg-[#0b101b]/90 border border-white/8 hover:border-white/16 rounded-2xl p-4 sm:p-5 flex flex-col gap-3.5 transition-all shadow-md">
                  {/* Header */}
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      <ExchangeLogo id={ex.id} name={ex.name} />
                  ...
                  {/* Feature Tags */}
                  <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                    <span className="px-3 py-1 rounded-xl bg-white/4 border border-white/6 flex items-center gap-1.5">🔒 Read-only API</span>
                    <span className="px-3 py-1 rounded-xl bg-white/4 border border-white/6 flex items-center gap-1.5">🔄 Portfolio Sync</span>
                    <span className="px-3 py-1 rounded-xl bg-white/4 border border-white/6 flex items-center gap-1.5">⏱ Trade History</span>
                  </div>
                  ...
                  {/* Action Button (Positioned tightly under description with zero gap) */}
                  <div className="pt-1">
                    <button
                      onClick={() => setConnecting(ex)}
                      className="w-full py-3 rounded-xl bg-[#061e2b]/80 border border-cyan-500/40 text-cyan-400 text-xs font-bold hover:bg-cyan-500/20 hover:border-cyan-500/60 transition-all flex items-center justify-center gap-1.5 shadow-sm"
                    >
                      Connect Account →
                    </button>
                  </div>
        ...
        {/* Bottom Info Strip */}
        <div className="bg-[#06151f]/80 border border-cyan-500/20 rounded-2xl px-4 sm:px-5 py-3.5 flex items-center justify-between gap-2 text-xs text-muted-foreground shadow-sm mt-1">
          <div className="flex items-center gap-2">
